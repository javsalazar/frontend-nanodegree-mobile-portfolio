...
var resizePizzas = function(size) {
    ...
    // Returns the class to apply to parent container. Called by changePizzaSlices(size).
    ...
    // don't need to iterate through all images since we are changing css style
    var newClass = sizeSwitcher(size) + " row";
...
function updatePositions() {
    ...
    //cache loop length, scrolled position
    var itemLength = pizzaList.length,
            scrolled = document.body.scrollTop,
            phase = [];

    // phase repeats every 5 instances so only calculate 5 and use 
    for (var i = 0; i < 5; i++) {
        phase.push(Math.sin(scrolled / 1250 + i) );
    }

    for (var j = 0; j < itemLength; j++) {
        // use transform instead of left to only trigger composite
        pizzaList[j].style.transform = 'translate3d(' +  500 * phase[j%5] + 'px, 0, 0)';
    }
...
// cache value, potentially called more than once
var movingPizzaContainer = document.getElementById("movingPizzas1"),
        pizzaList = [];

//create function for creating background pizzas since it will be used more than once.
function initPizzas () {
    'use strict';
    // we do not need 200 pizzas!, only enough to cover viewport
    var view = {
            width: window.innerWidth || document.body.clientWidth,
            height: window.innerHeight || document.body.clientHeight
        },
        cols = Math.floor(view.width / 200),
        rows = Math.floor(view.height / 200),
        s = 200,
...
document.addEventListener('DOMContentLoaded', function() {
    initPizzas();
    //updatePositions(); dont need to update on initial view, hidden by pic anyway.
});

// update pizzas on resize (moved event from html page)
var selector =    document.getElementById("sizeSlider");
